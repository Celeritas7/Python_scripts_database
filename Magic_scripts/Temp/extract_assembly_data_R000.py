...
Output:
  - Individual JSON per file  → cleaned/<filename>.json
  - Combined JSON for all     → cleaned/_all_assembly_data.json
  - Summary CSV               → cleaned/_parts_summary.csv
"""
...
def extract_cell_comments(zip_file):
    ...
    These are stored in xl/comments*.xml files.
    """
    comments = []
    ...
    for cf in comment_files:
        ...
        if comment_list is not None:
            for c in comment_list.findall(f'{ns}comment'):
                ...
                # Extract text from all <t> elements inside <r> runs
                text_parts = []
...
def extract_threaded_comments(zip_file):
    ...
    These are stored in xl/threadedComments/*.xml
    """
    comments = []
    all_files = zip_file.namelist()
    ...
    for tf in threaded_files:
        ...
        for tc in root.findall(f'{ns}threadedComment'):
            cell_ref = tc.get('ref', '?')
            # personId is a GUID — we just extract text
...
def extract_note_shapes(zip_file):
    """
    Extract notes embedded as VML shapes (legacy notes in xl/drawings/vmlDrawing*.vml).
    Older Excel versions store notes as VML shapes.
    """
    comments = []
    all_files = zip_file.namelist()
    ...
    for vf in vml_files:
        ...
        # VML can have encoding issues, try to parse
        try:
            # VML is not always well-formed XML, use regex fallback
            text_content = xml_bytes.decode('utf-8', errors='replace')
            # Find all text inside <div> tags (where VML notes store text)
...
def classify_texts(all_texts):
    ...
    for text in all_texts:
        ...
        # Check if it's a numbered step: "1) ...", "2) ..."
        ...
        # Check if it contains a part number
        ...
        # Check if it looks like a standalone part name
        is_likely_part_name = (
            len(text) < 50
        ...
        # Otherwise it's a note/caution — attach to current step
        if current_step:

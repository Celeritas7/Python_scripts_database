...
def classify_texts(all_texts):
    ...
    for text in all_texts:
        ...
        if is_likely_part:
            existing = {(p.get("name") or "").lower() for p in parts}
    ...
    quoted += re.findall(r'\u201c([^\u201d]+)\u201d', all_instr)
    quoted += re.findall(r'\u300c([^\u300d]+)\u300d', all_instr)
    existing_lower = {(p["name"] or "").lower() for p in parts if p.get("name")}
    for qp in quoted:
        qp = qp.strip()
    ...
    unique = []
    for p in parts:
        key = (p.get("name") or "", p.get("part_number") or "")
        if key not in seen:
            seen.add(key)
            unique.append(p)
...
def write_data_sheet(filepath, assembly_steps, parts, comments):
    ...
    if parts:
        for idx, part in enumerate(parts, 1):
            ...
            ws.cell(row=row, column=2, value=part.get("name") or "").font = normal_font
